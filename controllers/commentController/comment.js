...
const userId= req.userId
if(vote=="upvote"){
 const upvotedComment = await comments.findOne({_id:commentId,"votes.userId":userId,"votes.action":"upvote"})
 const downvotedComment=await comments.findOne({_id:commentId,"votes.userId":userId,"votes.action":"downvote"})
if(!upvotedComment){
  if(downvotedComment){
